...
- Shutdown tracking starts at `OnDispose`, records late `Settings.coc` writes through process shutdown, traces `SettingAsset.Save(...)` / `SaveWithPersist(...)`, `AssetDatabase.ProcessSingleSettingsFile(...)`, and `SaveSettingsHelper.Write*` activity, adds low-level `FileStream` / `File.Move` / `File.Replace` tracing for direct `Settings.coc` writes that bypass `SettingAsset`, groups late file changes into write episodes, logs the latest `Settings.coc` asset-session and stream-session context so you can distinguish one multi-step write from newly opened save sessions, and attempts a final conservative restore when the shutdown-stable file still meets the hard corruption threshold.
