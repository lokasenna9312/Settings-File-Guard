...
- Shutdown tracking starts at `OnDispose`, records late `Settings.coc` writes through process shutdown, traces `SettingAsset.Save(...)` / `SaveWithPersist(...)` entry and exit events, adds low-level `FileStream` / `File.Move` / `File.Replace` tracing for direct `Settings.coc` writes that bypass `SettingAsset`, and attempts a final conservative restore when the shutdown-stable file still meets the hard corruption threshold.
